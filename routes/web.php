...
//----4、视图加载  !! 知识点phpStorm 视图输入 ！+ tab 可以快速生成html头文件等代码  输入div+tab也可以快速生成标签
...
//通过控制器加载视图(常用)
Route::get('viewTest3',[TaskController::class,'index']);

//----5、路由分组 与 路由命名  ！！！知识点 按住ctrl点函数方法 可以跳转到函数定义部分
//PS url 是 uri 的子集   路由命名规范：控制器.方法
Route::get('task/url',[TaskController::class,'url'])->name('task.url'); //控制器中使用助手函数route('name') 可以获取到这个别名路由的url地址 http://localhost:8000/task/url
...
//----6、回退 当前路由 单行为
//单行为控制器  php artisan make:controller OneController --invoker
...
//----9、资源控制器 - 有一种控制器专门处理CURD(增删改查)，方法很多且方法名基本固定 -- 对于可以设置为资源控制器 不要大量设置路由
